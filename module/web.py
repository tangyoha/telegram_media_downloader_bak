...
def update_download_status(message_id: int,
                           down_byte: int,
                           total_size: int,
                           file_name: str,
                           start_time: float):
    ...
    if _download_result.get(message_id):
        ...
        if cur_time - last_time >= 1.0:
            download_speed = int(each_second_total_download /
                                 (cur_time - last_time))
            end_time = cur_time
        ...
        _download_result[message_id] =\
            {"down_byte": down_byte, "total_size":
             total_size, "file_name": file_name, "start_time": start_time,
             "end_time": end_time, "download_speed": download_speed,
        ...
        _download_result[message_id] = \
            {"down_byte": down_byte, "total_size":
             total_size, "file_name": file_name, "start_time": start_time,
             "end_time": cur_time, "download_speed": down_byte/(cur_time - start_time),
             "each_second_total_download": each_second_total_download}
...
def get_download_list():
    ...
    for idx, value in download_result.items():
        ...
        if result != '[':
            result += ','
        download_speed = format_byte(
            value["download_speed"]) + "/s"
        result += '{ "id":"' + \
            f'{idx}' + '", "filename":"' + os.path.basename(
                value["file_name"]) + '", "total_size":"' + \
            f'{format_byte(value["total_size"])}' + \
            '" ,"download_progress":"'
        result += str(round(value["down_byte"] /
                            value["total_size"] * 100, 2)) + \
            '" ,"download_speed":"' + \
            download_speed + '" ,"save_path":"' + \
